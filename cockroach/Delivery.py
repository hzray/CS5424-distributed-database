    ...
    def delivery_handler(self):
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute("""UPDATE CS5424.orders SET O_CARRIER_ID = %s 
                                WHERE O_ID IN %s AND O_W_ID = %s AND O_D_ID IN %s""",
                        (self.carrier_id, tuple(self.x), self.w_id, tuple(self.d)))
            # step c
            cur.execute("""UPDATE CS5424.order_line SET OL_DELIVERY_D = %s  
                               WHERE OL_O_ID IN %s AND OL_W_ID = %s AND OL_D_ID IN %s""",
                        (datetime.now(), tuple(self.x), self.w_id, tuple(self.d)))
        self.conn.commit()
        ...
        with self.conn.cursor() as cur:
            cur.execute("""SELECT SUM(OL_AMOUNT), OL_D_ID FROM CS5424.order_line 
                        ...
                        (tuple(self.x), self.w_id, tuple(self.d)))
            rows = cur.fetchall()
        self.conn.commit()
        ...
        with self.conn.cursor() as cur:
            cur.execute("""SELECT O_C_ID, O_D_ID FROM CS5424.orders WHERE O_ID IN %s AND O_W_ID = %s AND O_D_ID IN %s""",
                        (tuple(self.x), self.w_id, tuple(self.d)))
            rows = cur.fetchall()
        self.conn.commit()
