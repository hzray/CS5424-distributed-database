...
class NewOrder:
    ...
    def new_order_handler(self):
        ...
        for item in self.items:
            ...
            with self.conn.cursor() as cur:
                cur.execute("""SELECT S_QUANTITY, S_YTD, S_ORDER_CNT, S_REMOTE_CNT, S_DIST_{} 
            ...
            if self.w_id == item.warehouse_id:
                s_remote_cnt + 1
            with self.conn.cursor() as cur:
                cur.execute("""UPDATE CS5424.stock SET S_QUANTITY = %s, S_YTD = %s, S_ORDER_CNT = %s, 
                                S_REMOTE_CNT = %s WHERE S_I_ID = %s AND S_W_ID = %s""",
                            (adjusted_qty, s_ytd + quantity, s_order_cnt + 1, s_remote_cnt, item.item_id,
                             item.warehouse_id))
            self.conn.commit()
            # count total_amount
