...
CREATE TABLE CS5424.district (
  ...
  INDEX (D_W_ID, D_ID) STORING (D_NEXT_O_ID, D_NAME),
...
CREATE TABLE CS5424.customer (
  ...
  INDEX (C_W_ID, C_D_ID, C_ID, C_BALANCE) STORING (C_FIRST, C_MIDDLE, C_LAST),
  INDEX (C_W_ID, C_D_ID, C_ID) STORING (C_DISCOUNT, C_LAST, C_CREDIT),
...
CREATE TABLE CS5424.order_line (
  ...
  INDEX (OL_W_ID, OL_D_ID, OL_O_ID, OL_I_ID, OL_QUANTITY) STORING (OL_AMOUNT),
...
UPDATE CS5424.order_line SET OL_I_NAME = (SELECT I_NAME FROM CS5424.item WHERE I_ID = OL_I_ID) WHERE TRUE;